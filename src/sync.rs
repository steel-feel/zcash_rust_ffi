...
pub async fn sync(wallet_name: String) -> Result<(), anyhow::Error> {
    ...
    let _chain_tip = update_chain_tip(&mut client, &mut db_data).await?;
    ...
    loop {
        ...
        match scan_ranges.first() {
            Some(scan_range) if scan_range.priority() == ScanPriority::Verify => {
                ...
                let block_meta =
                    download_blocks(&mut client, &fsblockdb_root, &db_cache, &scan_range).await?;
...
async fn refresh_utxos<P: Parameters>(
    ...
    if request.addresses.is_empty() {
        println!("{:?} has no transparent receivers", account_id);
    } else {
        client
