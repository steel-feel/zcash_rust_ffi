...
pub fn txn_list(wallet_name: String, account_id: Option<Uuid>,) -> Result<(), anyhow::Error> {
    
      let wallet_dir: Option<String> = Some(wallet_name.to_owned());
        let (_, db_data) = get_db_paths(wallet_dir);
        let mode = ListMode::Text;
        ...
        for row in stmt_txs.query_and_then(
            named_params! {":account_uuid": account_id },
            |row| -> anyhow::Result<_> {
...
impl WalletTxOutput {
    ...
    fn print_csv(&self, context: &WalletTx) -> Result<(), anyhow::Error> {
        ...
        if let Some(action) = match (&self.from_account, &self.to_account) {
            ...
            let symbol = "ZEC";
            let volume = self.value.into_u64();
            let currency = "USD";
            ...
            let total = "";
            let price = "";
            let fee = context.fee_paid.expect("error fee").into_u64();
...
impl WalletTx {
    ...
    fn print_text(&self) {
        ...
        println!("    Amount: {:?}", self.account_balance_delta );
        println!(
            "  Fee paid: {:?}",
            self.fee_paid
        );
